package br.com.fiap.ecocompliance.controller;

import br.com.fiap.ecocompliance.dto.AuditoriaRequestDTO;
import br.com.fiap.ecocompliance.dto.AuditoriaResponseDTO;
import br.com.fiap.ecocompliance.entity.NivelRisco;
import br.com.fiap.ecocompliance.exception.ResourceNotFoundException;
import br.com.fiap.ecocompliance.service.AuditoriaService;
import com.fasterxml.jackson.databind.ObjectMapper;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.WebMvcTest;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.context.annotation.Import;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.Pageable;
import org.springframework.http.MediaType;
import org.springframework.security.test.context.support.WithMockUser;
import org.springframework.test.web.servlet.MockMvc;

import java.time.LocalDate;
import java.util.List;

import static org.mockito.ArgumentMatchers.*;
import static org.mockito.Mockito.*;
import static org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.csrf;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

@WebMvcTest(AuditoriaController.class)
@Import(br.com.fiap.ecocompliance.config.SecurityConfig.class)
class AuditoriaControllerTest {

    @Autowired MockMvc mockMvc;
    @Autowired ObjectMapper objectMapper;

    @MockBean AuditoriaService auditoriaService;
    @MockBean br.com.fiap.ecocompliance.security.JwtService jwtService;
    @MockBean br.com.fiap.ecocompliance.repository.UsuarioRepository usuarioRepository;

    private AuditoriaResponseDTO responseValido() {
        return new AuditoriaResponseDTO(
                1L, 1L, "Verde Industria S.A.",
                LocalDate.now(), "Conformidade parcial",
                "Ajustes necessários", NivelRisco.MEDIO);
    }

    private AuditoriaRequestDTO requestValido() {
        return new AuditoriaRequestDTO(
                1L, LocalDate.now(), "Conformidade parcial",
                "Ajustes necessários", NivelRisco.MEDIO);
    }

    @Test
    @DisplayName("listar com usuário autenticado deve retornar 200 com página")
    @WithMockUser(roles = "USER")
    void listar_comUserAutenticado_retorna200() throws Exception {
        when(auditoriaService.listar(any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(responseValido())));

        mockMvc.perform(get("/auditorias"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray())
                .andExpect(jsonPath("$.content[0].resultado").value("Conformidade parcial"))
                .andExpect(jsonPath("$.content[0].nivelRisco").value("MEDIO"));
    }

    @Test
    @DisplayName("listar sem autenticação deve retornar 4xx")
    void listar_semAutenticacao_retorna4xx() throws Exception {
        mockMvc.perform(get("/auditorias"))
                .andExpect(status().is4xxClientError());
    }

    @Test
    @DisplayName("buscar por ID com usuário autenticado deve retornar 200")
    @WithMockUser(roles = "USER")
    void buscarPorId_comUserAutenticado_retorna200() throws Exception {
        when(auditoriaService.buscarPorId(1L)).thenReturn(responseValido());

        mockMvc.perform(get("/auditorias/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.nivelRisco").value("MEDIO"));
    }

    @Test
    @DisplayName("buscar por ID quando não existe deve retornar 404")
    @WithMockUser(roles = "USER")
    void buscarPorId_quandoNaoExiste_retorna404() throws Exception {
        when(auditoriaService.buscarPorId(99L))
                .thenThrow(new ResourceNotFoundException("Auditoria não encontrada com ID: 99"));

        mockMvc.perform(get("/auditorias/99"))
                .andExpect(status().isNotFound());
    }

    @Test
    @DisplayName("buscar por risco com usuário autenticado deve retornar 200")
    @WithMockUser(roles = "USER")
    void buscarPorRisco_comUserAutenticado_retorna200() throws Exception {
        when(auditoriaService.buscarPorRisco(NivelRisco.ALTO))
                .thenReturn(List.of(new AuditoriaResponseDTO(
                        2L, 1L, "Verde Industria S.A.",
                        LocalDate.now(), "Não conformidade grave", null, NivelRisco.ALTO)));

        mockMvc.perform(get("/auditorias/risco/ALTO"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].nivelRisco").value("ALTO"));
    }

    @Test
    @DisplayName("criar com admin autenticado deve retornar 201")
    @WithMockUser(roles = "ADMIN")
    void criar_comAdminAutenticado_retorna201() throws Exception {
        when(auditoriaService.criar(any())).thenReturn(responseValido());

        mockMvc.perform(post("/auditorias")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(requestValido())))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id").value(1));
    }

    @Test
    @DisplayName("escrita com usuário sem permissão deve retornar 403")
    @WithMockUser(roles = "USER")
    void criar_comUserSemPermissao_retorna403() throws Exception {
        mockMvc.perform(delete("/auditorias/1")
                        .with(csrf()))
                .andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("excluir com admin autenticado deve retornar 204")
    @WithMockUser(roles = "ADMIN")
    void excluir_comAdminAutenticado_retorna204() throws Exception {
        doNothing().when(auditoriaService).excluir(1L);

        mockMvc.perform(delete("/auditorias/1")
                        .with(csrf()))
                .andExpect(status().isNoContent());
    }
}
